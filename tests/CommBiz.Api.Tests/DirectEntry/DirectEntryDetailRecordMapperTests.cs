...
public class DirectEntryDetailRecordMapperTests
{
    ...
    public void Each_field_lands_at_its_documented_character_position()
    {
        ...
        Assert.Equal("484-799", record[1..8]); // BSB Number (payload Destination), position 2-8, length 7
        Assert.Equal("300500", record[8..17].Trim()); // Account Number (payload Destination), position 9-17, length 9
        ...
        Assert.Equal("JOHN CITIZEN", record[30..62].TrimEnd()); // Title (payload Destination), position 31-62, length 32
        Assert.Equal("PAYMENTS", record[62..80].TrimEnd()); // Lodgement Reference, position 63-80, length 18
        Assert.Equal("062-000", record[80..87]); // Trace BSB Number (Trace setting), position 81-87, length 7
    ...
    [Fact]
    public void Bsb_is_reformatted_from_the_payloads_raw_6_digits_to_nnn_dash_nnn()
    {
    ...
    [Fact]
    public void Trace_account_number_setting_shorter_than_9_chars_is_right_justified_space_filled()
    {
        var record = DirectEntryDetailRecordMapper.Map(
    ...
    [Fact]
    public void Trace_account_number_setting_at_max_length_needs_no_padding()
    {
        var record = DirectEntryDetailRecordMapper.Map(
    ...
    {
        var record = DirectEntryDetailRecordMapper.Map(
            ValidInstruction() with { DestinationBankAccountName = new string('A', 40) }, Settings);
    ...
    [Fact]
    public void Remitter_name_setting_shorter_than_max_is_left_justified_space_filled()
    {
    ...
    [Fact]
    public void Remitter_name_setting_longer_than_max_is_truncated_to_keep_the_record_fixed_width()
    {
        var record = DirectEntryDetailRecordMapper.Map(
            ValidInstruction(), Settings with { NameOfRemitter = new string('A', 20) });
    ...
    public void MapFields_cba_response_values_match_the_same_values_written_into_the_text_record()
    {
        ...
        Assert.Equal("484-799", fields.Single(f => f.CbaResponseField == "BSB Number").CbaResponseValue);
        Assert.Equal("062-000", fields.Single(f => f.CbaResponseField == "Trace BSB Number").CbaResponseValue);
    ...
    [Fact]
    public void MapFields_payload_sourced_bsb_number_field_carries_the_raw_unformatted_request_value()
    {
        var fields = DirectEntryDetailRecordMapper.MapFields(ValidInstruction(), Settings);

        Assert.Equal("484799", fields.Single(f => f.CbaResponseField == "BSB Number").RequestValue);
        Assert.Equal(
    ...
    [Fact]
    public void MapFields_settings_sourced_trace_bsb_field_is_attributed_to_the_trace_settings_field()
    {
        var fields = DirectEntryDetailRecordMapper.MapFields(ValidInstruction(), Settings);

        Assert.Equal("062-000", fields.Single(f => f.CbaResponseField == "Trace BSB Number").RequestValue);
        Assert.Equal(
