...
| PM-006 | F-017 (IMT) and F-018 (Priority Payments) cannot be planned in detail yet — the exact upstream request-object JSON shape for each (analogous to BPAY's confirmed `PaymentTypeCode`/`AccountNo`/`PaymentSourceTypeCode`/`PaymentDate`/`Amount`/`BPayBillerCode`/`BPayReference` shape) has not been provided. The Payment Type Router (F-015) is already extensible — adding either type once its shape is confirmed is a small additive change, not a redo. | User | Before F-017/F-018 can start |
